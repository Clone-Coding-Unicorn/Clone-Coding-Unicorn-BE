...
//}
    ...
    public List<PostResponseDto> getSortedPostList(String category) { //카테고리별 포스트 조회
        List<Post> sortedPost = postRepository.findAllByCategoryOrderByDateDesc(category); // 최신순으로 정렬
...
//                                LocalDate targetDate = LocalDate.parse("2023.07.18", formatter);
            ...
            for (Category newsDetailsLinkPair : todaysNewsLinkList) {
                ...
                postRepository.save(
                        new Post(newsTitle, contents, firstImageUrl,
                                newsDate, db_category)
                );
            }
